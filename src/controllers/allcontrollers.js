...
// Upload inspection photo to Cloudinary and save to database
const uploadInspectionPhoto = async (req, res) => {
  inspectionPhotoUpload.single('photo')(req, res, async (err) => {
    try {
      if (err) {
        return res.status(400).json({
          success: false,
      ...
      const { inspectionToken, sectionId, questionId, subQuestionId } = req.body;
      ...
      if (!inspectionToken || !sectionId || !questionId) {
        return res.status(400).json({
          success: false,
          error: 'Inspection token, section ID, and question ID are required'
        });
      }

      // Find the inspection
      const inspection = await Inspection.findOne({ accessToken: inspectionToken });
      if (!inspection) {
        return res.status(404).json({
          success: false,
      ...
      const uploadOptions = {
        folder: `vos-inspections/${inspectionToken}/${sectionId}`,
        public_id: `${questionId}_${subQuestionId ? `${subQuestionId}_` : ''}${Date.now()}`,
        transformation: [
          { width: 1200, height: 1200, crop: 'limit', quality: 'auto' }
        ]
      };

      console.log(`Uploading inspection photo to Cloudinary for inspection ${inspectionToken}`);
      ...
      // Create photo object
      const photoData = {
        path: cloudinaryResult.secure_url, // For backward compatibility
      ...
      // Add photo to the appropriate location
      if (subQuestionId) {
        // Add to sub-question
        ...
        if (!subQuestion.photos) {
          subQuestion.photos = [];
        }
        subQuestion.photos.push(photoData);
      } else {
        // Add to main question
        if (!question.photos) {
          question.photos = [];
        }
        question.photos.push(photoData);
      }

      // Save the inspection
      await inspection.save();

      console.log(`Inspection photo uploaded and saved successfully`);
...
// Delete inspection photo from Cloudinary and database
const deleteInspectionPhoto = async (req, res) => {
  try {
    const { inspectionToken, sectionId, questionId, subQuestionId, photoIndex } = req.body;

    if (!inspectionToken || !sectionId || !questionId || photoIndex === undefined) {
      return res.status(400).json({
        success: false,
        error: 'Inspection token, section ID, question ID, and photo index are required'
      });
    }
    ...
    if (subQuestionId) {
      ...
      const subQuestion = question.subQuestions.find(sq => sq.id === subQuestionId);
      if (!subQuestion || !subQuestion.photos) {
        return res.status(404).json({
          success: false,
          error: 'Sub-question or photos not found'
        });
      }
      photos = subQuestion.photos;
    } else {
    ...
    if (photoToDelete.publicId) {
      try {
        ...
        console.warn('Failed to delete photo from Cloudinary:', cloudinaryError);
    ...
    console.log(`Inspection photo deleted successfully`);
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Server error while deleting photo'
    });
  }
};
...
exports.deleteInspectionPhoto = deleteInspectionPhoto;
